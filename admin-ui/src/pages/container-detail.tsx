...
export function ContainerDetailPage() {
  ...
  return (
    <div>
      <Header
        title={`Container ${container.id.slice(0, 12)}...`}
        actions={
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <Card>
            ...
            <CardContent className="space-y-2">
              ...
              <dl className="space-y-1 text-xs">
                <div className="flex gap-2">
                  <dt className="w-24 text-muted-foreground">Docker ID</dt>
                  <dd className="font-mono">{container.docker_id ?? 'N/A'}</dd>
                </div>
                <div className="flex gap-2">
                ...
                <div className="flex gap-2">
                  <dt className="w-24 text-muted-foreground">Last Activity</dt>
          ...
          <Card>
            ...
            <CardContent className="space-y-1 text-xs">
              <div className="flex gap-2">
                <dt className="w-28 text-muted-foreground">Task</dt>
                <dd className="max-w-60 truncate">{container.task}</dd>
              </div>
              <div className="flex gap-2">
                <dt className="w-28 text-muted-foreground">CPU / Memory</dt>
                <dd>
                  {container.cpu_limit} / {container.memory_limit}
                </dd>
              </div>
              <div className="flex gap-2">
                <dt className="w-28 text-muted-foreground">Idle Timeout</dt>
                <dd>{container.idle_timeout}s</dd>
              </div>
              <div className="flex gap-2">
                <dt className="w-28 text-muted-foreground">Max Lifetime</dt>
                <dd>{container.max_lifetime}s</dd>
              </div>
              {repos.length > 0 && (
                <div className="flex gap-2">
                  <dt className="w-28 text-muted-foreground">Skill Repos</dt>
                  <dd>
                    {repos.map((r, i) => (
